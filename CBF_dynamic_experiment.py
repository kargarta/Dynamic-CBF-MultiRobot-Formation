...
for center, size, vel in [([1.4, 0.6], [0.2,0.1], [-0.025, -0.00]), ([ 0.7,0.7], [0.2,0.1], [-0.000, -0.060])]:
...
x_goal = np.array([1.3, 0.0], dtype=np.float32)
...
for _ in range(1000):
    ...
    r.set_velocities(range(N), 0.1*dxu)
    r.step()
