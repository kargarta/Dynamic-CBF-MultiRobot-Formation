...
dyn_specs = [
    ([1.4, 0.6], [0.2,0.1], [-0.043, -0.00]),
    ([0.7,0.75], [0.2,0.1], [-0.000, -0.060])
]
...
# === Post-processing: CSV & plots ===
pairs = [f'{i}-{j}' for i in range(N) for j in range(i+1,N)]
inter_arr = np.array(inter_robot_dist)  # shape (T, C(N,2))

# obs_dist_min is now a list of lists -> array shape (T, N)
obs_arr = np.array(obs_dist_min, dtype=np.float64)   # shape (T, N)

df = pd.DataFrame({
    'step': range(len(center_distances)),
    'center_dist': center_distances,
    'form_error': formation_error,
})

# add obs-dist columns, one per robot (each value is numpy.float64)
for i in range(N):
    df[f'obs_dist_min_r{i}'] = obs_arr[:, i]

# add inter-robot distances as before
for idx,label in enumerate(pairs):
    df[f'dist_{label}'] = inter_arr[:, idx]

df.to_csv('Baseline1_with_dynamic.csv', index=False)
print("Saved Baseline1_with_dynamic.csv")

# Plot metrics
plt.figure(); plt.plot(center_distances); plt.xlabel('Step'); plt.ylabel('Center Distance'); plt.grid(True)
plt.figure(); plt.plot(formation_error); plt.xlabel('Step'); plt.ylabel('Formation Error'); plt.grid(True)
plt.figure()
for i in range(N):
    plt.plot(obs_arr[:, i], label=f'robot {i}')
plt.xlabel('Step'); plt.ylabel('Min Obstacle Dist (per robot)'); plt.legend(); plt.grid(True)

plt.figure()
for idx,label in enumerate(pairs): plt.plot(inter_arr[:,idx], label=label)
plt.xlabel('Step'); plt.ylabel('Inter-Robot Dist'); plt.legend(); plt.grid(True)
plt.show()
