...
init_center = np.array([-0.7, 0.04], dtype=np.float32)
...
init_radius = 8* target_formation_radius   # 0.5 * 0.1 *?? keep same calculation as original
...
for center, size, vel in [([ 0.95,-25], [0.2,0.1], [-0.000, 0.1]), ([ 1,20], [0.2,0.1], [-0.000, -0.06])]:
...
x_goal = np.array([1.5, 0.15], dtype=np.float32)
...
for _ in range(steps):
    ...
    r.set_velocities(range(N), 0.03*dxu)
    r.step()
